...
class ImageManager extends CApplicationComponent
{
	...
	/**
	 * Rebuild images cache for a Model which uses ImageBehavior
	 *
	 * @param string $modelName
	 */
	public function rebuildModelImages($modelName)
	{
		$items = $modelName::model()->findAll();
		foreach ($items as $item) {
			foreach ($item->behaviors() as $behavior) {
				if (isset($behavior['class']) and isset($behavior['prebuild'])
						and $behavior['class'] == 'common.behaviors.ImageBehavior' and is_array($behavior['prebuild'])) {
					foreach ($behavior['attributes'] as $attribute) {
						if (null !== $item[$attribute]) {
							foreach ($behavior['prebuild'] as $format) {
								$this->alias($item[$attribute], $format);
							}
						}
					}
				}
	 ...
	 *
	 * @param string $image
	 * @param boolean $keepOriginal	Delete also the uploaded file (default: true)
	 */
	public function deleteAll($image, $keepOriginal = false)
	{
		/*
		 * Iterate through formats
		 */
		if ($keepOriginal and file_exists($this->originalPath . $image)) {
