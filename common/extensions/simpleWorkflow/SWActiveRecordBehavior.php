...
class SWActiveRecordBehavior extends CBehavior {
	...
	protected function canFireEvent($owner,$className){
		return ( is_object($owner) and is_a($owner, $className) );
	}
	/**
	...
	private function _updateStatus($SWNode){
		if(!( is_object($SWNode) and is_a($SWNode,'SWNode')))
	...
	protected function initialize(){
		Yii::trace(__CLASS__.'.'.__FUNCTION__,self::SW_LOG_CATEGORY);
		if( is_object($this->getOwner()) and is_a($this->getOwner(), 'CActiveRecord')){
	...
	private function _runTransition($sourceSt,$destSt,$event=null){
		Yii::trace(__CLASS__.'.'.__FUNCTION__,self::SW_LOG_CATEGORY);
		if(!is_null($sourceSt) and is_object($sourceSt) and is_a($sourceSt,'SWNode')){
	...
	public function swValidate($attribute, $value){
        ...
        $bResult=false;
        try{
        	if(is_object($value) and is_a($value, 'SWNode')){
        		$swNode=$value;
        	} else {
